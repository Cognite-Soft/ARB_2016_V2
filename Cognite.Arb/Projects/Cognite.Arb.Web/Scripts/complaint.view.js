(function () {
    ...
    function GetCommentType(sender) {
        if (sender.hasClass("advice")) return "Advise";
    ...
    /*Pravahya:Save button binding*/
    var saveButtonSelector = "a.save-edited-post";
    ...
    function BindDiscussionsActions() {
        ...
        /*Pravahya:Save button binding function*/
        saveButtons.click(SaveEditedPost);
    }

    /*Pravahya:Save method for edited post*/
    function SaveEditedPost(e) {
        var sender = $(e.target);
        var postID = sender.attr('data-comment-id');
        var text = $('#' + postID).find('textarea').val();
        if (text != undefined && text != '') {
            SendEditedPost(postID, text);
        }
            //SendReplyOnPost(postId, text);
    }

    function SendEditedPost(postId, text) {
        var url = "/*url goes here*/" + modelId;
    ...
    $("#btnSendNotificationToCaseWorker").click(function () {

        var message = $("input:radio[name=Notify]:checked").val();

        var url = '/Notification/NotifyCaseWorker/' + modelId;
        
        if (message == undefined)
        {
            $("#divNotificationMessage").show();
            $("#divNotificationMessage").text("Please select any one of the message option");            
            return false;
        }
        $("#divNotificationMessage").hide();
        //$("input:radio[name=Notify]:checked").prop('checked',false);
        
        $.ajax({
            type: 'POST',
            url: url,
            data: {Id : modelId, Message : message ,__RequestVerificationToken: verificationToken},
            success: function (data) {
                $("#divNotificationMessage").show();
                $("#divNotificationMessage").text('Notification has been sent successfully');
                $("#divNotificationMessage").css("color","green");
            }
        });
    });
...
$(document).ready(function () {
    $("#btnclickmail").click(function () {
        alert('test');
        $.ajax({
            type: 'GET',
            url: '/Notification/NotifyCaseWorker/',                       
            success: function (data) {
                alert(data);
